...
const categoriesReducer = (state = initialState, action) => {
    switch (action.type) {
        ...
        case CHANGE_CURRENT_WAY_ADD:
            if (
                state.currentWay[state.currentWay.length - 1] !== action.newItem
            ) {
                return {
                    ...state,
...
export const changeAddCatText = (newText) => ({
    type: CHANGE_ADD_CAT_TEXT,
    newText,
});
export const changeListCurrentRoot = (catName) => ({
    type: CHANGE_LIST_CURRENT_ROOT,
    catName,
});

export const setCurrentRootCategories = (newList) => ({
    type: SET_CURRENT_ROOT_CATEGORIES,
    newList,
});

export const addToCurrentWay = (newItem) => ({
    type: CHANGE_CURRENT_WAY_ADD,
    newItem,
});
...
//getting and set all subcategories by category name
...
export const addCategoryToDB = (_id, parent) => async (dispatch) => {
    let data;
    try {
...
export const deleteCategoryFromDB = (id) => async () => {
    try {
        const data = await request("/api/cat/delete" + id, "DELETE")
        return true;
    } catch (e) {}
    return false;
};
