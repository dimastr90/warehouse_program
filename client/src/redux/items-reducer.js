...
export const setAllCategories = (arrayWithCat) => ({type: SET_ALL_CATEGORIES, arrayWithCat});
export const setAllItems = (arrayWithItems) => ({type: SET_ALL_ITEMS, arrayWithItems});
...
export const addCatToBase = (category, name, price) => async (dispatch) => {
    try {
        const data = await request('api/items/put/', 'PUT', {category, name, price});
        if (data) {
...
export const deleteItemFromBase = (id, category) => async (dispatch) => {
    try {
...
export const editCatInBase = (id, category, name, price, oldCat) => async (dispatch) => {
    try {
        await request(`/api/items/replace/`, 'PUT', {id,category,name,price,oldCat});
        return true;
    } catch (e) {
...
export const renameItemInBase = (id, category, name, price) => async (dispatch) => {
    try {
        await request(`/api/items/rename/item`, 'PUT', {id,category,name,price});
        return true;
    } catch (e) {
