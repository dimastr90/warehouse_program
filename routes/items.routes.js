...
router.put(
    ...
    [
        ...
        check("price", "Min parent price length: 1")
            .isLength({ min: 1 })
            .isNumeric(),
    ],
    async (req, res) => {
...
//get all categories list with their items
...
router.get("/getitems/", async (req, res) => {
    try {
        const data = await Categories.find();
        let result = data.reduce((accum, i) => {
            return [...accum, ...i.items];
        }, []);
