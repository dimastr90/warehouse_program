...
//get all bases list, and send it to front-end
...
router.post(
    ...
    [
        ...
        check("name", "Min name name length: 1").isLength({ min: 1 }),
        check("price", "Min name price length: 1")
            .isLength({ min: 1 })
            .isNumeric(),
    ...
    async (req, res) => {
        try {
            ...
            const check = await Bases.find({
                $and: [
                    { _id: base },
                    { items: { $elemMatch: { uniq_Id: id } } },
                ],
            });
...
router.put(
    ...
    [
        ...
        check("price", "Min name price length: 1")
            .isLength({ min: 1 })
            .isNumeric(),
        check("qty", "Min parent qty length: 1")
            .isLength({ min: 1 })
            .isNumeric(),
    ],
    async (req, res) => {
        try {
            ...
            res.status(201).json({ message: "Category item is changed" });
        } catch (e) {
            res.status(500).json({ message: "Something went wrong" });
        }
    }
